...
function parseInput {
    ...
    # $source = @('R8,U5,L5,D3',
    #             'U7,R6,D4,L4')
    # $source = @('R75,D30,R83,U83,L12,D49,R71,U7,L72',
    #             'U62,R66,U55,R34,D71,R55,D58,R83')
    # $source = @('R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51',
    #             'U98,R91,D20,R16,D67,R40,U7,R15,U6,R7')

    $data = foreach ($line in $source) {
        $points = foreach($p in $line -split ',') {
            if ($p -notmatch '^([RULD])(\d+)$') { throw $p }
            $dx,$dy = 0,0
            switch($Matches[1]) {
                'U' {
                    $dy = +1
...
function part1 {
    ...
    foreach ($line in $data) {
        ++$lineNo;
        $pos = [pscustomobject]@{x=0; y=0}
        foreach ($d in $line.Points) {
            for ($i = 0; $i -lt $d.n; ++$i) {
                $pos.x += $d.dx
                $pos.y += $d.dy
                $xy = "$($pos.x),$($pos.y)"
                if (!$path.ContainsKey($xy)) {
                    $path[$xy] = $lineNo
                }
                elseif ($path[$xy] -ne $lineNo) {
    ...
    foreach ($line in $data) {
        ...
        foreach ($d in $line.Points) {
            for ($i = 0; $i -lt $d.n; ++$i) {
                ...
                ++$step
                $xy = "$($pos.x),$($pos.y)"
                if (!$lines[$lineNo].ContainsKey($xy)) {
                    $lines[$lineNo][$xy] = $step
                }
                ...
                elseif ($path[$xy] -ne $lineNo) {
                    # crosses another line
                    $steps = $lines[$path[$xy]][$xy] + $step
                    if ($minSteps -gt $steps) {
                        $minSteps = $steps
                    }
                }
            }
